...
//  RobotController
//
//  Created by Jenna Luchak on 2018-07-08.
...
/*
 A function that will receive updates from the JoyStick when the handle moves. Takes two values, both CGFloats.
 ...
 */
public typealias JoyStickMonitor = (_ angle: CGFloat, _ displacement: CGFloat, _ stopTouch: CGFloat) -> ()  //CGFloat

/**
 As the joystick moves, two values are detected and reported:
 ...
 */
public final class JoyStick: UIView {
    
    // override class var requiresConstraintBasedLayout: Bool { return true }
    
    /// Holds a function to call when joystick orientation changes
    public var monitor: JoyStickMonitor? = nil
    ...
    public private(set) var displacement: CGFloat = 0
    
    /// --- ////
    public private(set) var stopTouch: CGFloat = 0
    ...
    private let baseImage = UIImage(named: "testing")! //JoyStickBase
    // clock from http://www.tarifi.info/imamstutz-%D0%A6%D0%B8%D1%84%D0%B5%D1%80%D0%B1%D0%BB%D0%B0%D1%82-%D0%A7%D0%B0%D1%81%D0%BE%D0%B2-Png.alp
    /// The image to use for the joystick handle
    private let handleImage = UIImage(named: "JoystickHandle")! //JoyStickHandle
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    /** UNCOMMENT IF YOU WANT TO RETURN TO ORIGINAL POSITION AFTER RELEASING TOUCH
    ...
    /**
     When the user removes their finger from the joystick, a variable should change
     */
     public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
       stopTouchYES()
    }
    ...
    //
    //
    private func stopTouchYES() {
        stopTouch = 1;
        monitor?(angle, displacement, stopTouch)
    }
    private func stopTouchNO() {
        stopTouch = 0;
        monitor?(angle, displacement, stopTouch)
    }
    ...
    private func updateLocation(location: CGPoint) {
        ...
        if newClampedDisplacement != displacement || newAngleRadians != lastAngleRadians {
            ...
            //
            self.angle = newClampedDisplacement != 0 ? CGFloat(180 - newAngleRadians * 180 / Float.pi) : 0
            monitor?(angle, displacement, stopTouch)
        }
    }
}
